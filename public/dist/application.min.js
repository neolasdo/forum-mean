"use strict";var ApplicationConfiguration=function(){var applicationModuleName="mean",applicationModuleVendorDependencies=["ngResource","ngAnimate","ngMessages","ui.router","ui.bootstrap","ui.utils","angularFileUpload","toastr","asideModule","ui.select"],registerModule=function(moduleName,dependencies){angular.module(moduleName,dependencies||[]),angular.module(applicationModuleName).requires.push(moduleName)};return{applicationModuleName:applicationModuleName,applicationModuleVendorDependencies:applicationModuleVendorDependencies,registerModule:registerModule}}();
...
!function(app){"use strict";app.registerModule("group")}(ApplicationConfiguration);
...
"use strict";angular.module("core").controller("HomeController",["$scope","$http","Authentication","$state","toastr","$modal",function($scope,$http,Authentication,$state,toastr,$modal){var vm=this;vm.auth=Authentication,vm.showAll=!1,vm.showAllGroup=function(){vm.showAll=!vm.showAll},vm.getJoinedGroup=function(){$http.get("/api/"+vm.auth.user.roles+"/"+vm.auth.user._id+"/groups/getAllJoined").success(function(res){}).error(function(err){})},vm.getJoinedGroup(),vm.getMyGroup=function(){"teacher"==vm.auth.user.roles&&$http.get("/api/teacher/"+vm.auth.user._id+"/groups/getAllByUser").success(function(res){}).error(function(err){})},vm.getMyGroup(),vm.addGroup=function(){$modal.open({animation:!1,templateUrl:"modules/core/client/views/addGroupModal.html",controller:"AddGroupController",controllerAs:"vm",size:"lg",windowClass:"AddGroupController"})},""==vm.auth.user&&$state.go("authentication.signin")}]).controller("AddGroupController",["$scope","$http","Authentication","$state","toastr","$modal","$modalInstance","$timeout","$window","FileUploader",function($scope,$http,Authentication,$state,toastr,$modal,$modalInstance,$timeout,$window,FileUploader){var vm=this;vm.teachers=[],vm.imageURL="",vm.getListTeacher=function(){$http.get("/api/users/getAllTeacher").then(function(res){vm.teachers=res.data;var index=vm.teachers.findIndex(function(teacher){return teacher._id===Authentication.user._id});vm.teachers.splice(index,1)})},vm.getListStudent=function(){$http.get("/api/users/getAllStudent").then(function(res){vm.students=res.data})},vm.getListTeacher(),vm.getListStudent(),vm.group={},vm.user=Authentication.user,vm.close=function(){$modalInstance.close()},vm.uploader=new FileUploader({url:"/api/groups/picture",alias:"newGroupPicture"}),vm.uploader.filters.push({name:"imageFilter",fn:function(item,options){var type="|"+item.type.slice(item.type.lastIndexOf("/")+1)+"|";return"|jpg|png|jpeg|bmp|gif|".indexOf(type)!==-1}}),vm.uploader.onAfterAddingFile=function(fileItem){if($window.FileReader){var fileReader=new FileReader;fileReader.readAsDataURL(fileItem._file),fileReader.onload=function(fileReaderEvent){$timeout(function(){vm.error=null,vm.imageURL=fileReaderEvent.target.result,vm.uploader.uploadAll()},0)}}},vm.uploader.onSuccessItem=function(fileItem,response,status,headers){vm.imageURL=response,vm.uploader.clearQueue()},vm.uploader.onErrorItem=function(fileItem,response,status,headers){vm.uploader.clearQueue(),vm.error=response.message,vm.imageURL=""},vm.save=function(){vm.imageURL&&(vm.group.image=vm.imageURL),vm.user&&(vm.group.createdBy=vm.user._id),vm.group&&$http.post("/api/addGroup",vm.group).success(function(res){"success"==res.status&&(vm.hasError=!1,toastr.success("Lớp học đã được tạo thành công"),vm.close(),$state.reload())}).error(function(err){vm.hasError=!0,vm.errorMessages=err})}}]);
